...
<cfquery datasource="#application.dsn#" name="gettop5">
SELECT     TOP (5) IdeaID, SUM(CASE type WHEN 'Up' THEN 1 ELSE 0 END) AS UpVote, SUM(CASE type WHEN 'Down' THEN 1 ELSE 0 END) AS DownVote, 
                      SUM(CASE type WHEN 'Up' THEN 1 ELSE 0 END) - SUM(CASE type WHEN 'Down' THEN 1 ELSE 0 END) AS NetVotes
FROM         bizmag_ideas_votes
GROUP BY IdeaID
ORDER BY netvotes DESC
</cfquery>
<cfquery datasource="#application.dsn#" name="ideas">
SELECT    ID, Title, Body, Author, Image
FROM         bizmag_ideas
WHERE ID IN (#ValueList(gettop5.IdeaID)#)
</cfquery>
